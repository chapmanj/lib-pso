...
namespace ParticleSwarmLibrary
{
    public class PSO
    {
        ...
        public List<float> Run(ulong NumberOfIterations, int NumberOfParticles)
        {
            ...
            Console.WriteLine("\t^ indicates new global best found solution");
            ...
            for (ulong i = 0; i < NumberOfIterations; i++)
            {
                ...
                //get min ofv
                Particle iterationBestOFV = particles.OrderBy(part => part.getObjectiveFunctionValue()).First();
                
                //update if better
                if (iterationBestOFV.getObjectiveFunctionValue() < getGlobalBestObjectiveFunctionValue())
                {
                    globalBestObjectiveFunctionValue = iterationBestOFV.getObjectiveFunctionValue();

                    globalBestPositionVector = iterationBestOFV.getPositionVector().ToList();

                    //optional feedback - global
                    if (i % 100 == 0)
                        Console.WriteLine(i + "\t^\t" + iterationBestOFV.ToString());
                }
